...
var opt = ((((require('node-getopt')).create([['h', 'help', 'display this help'],
    ...
    ['i', 'include-dir=ARG+', 'add directory to include search path']])).setHelp(("elfenben [OPTION] [<infile>] [<outfile>]\n\n" + "<outfile> will default to <infile> with '.js' extension\n\n" + "Also compile stdin to stdout\n" + "eg. $ echo '(console.log \"hello\")' | elfenben\n\n" + "[[OPTIONS]]\n\n"))).bindHelp()).parseSystem();
(function(___monad) {
    ...
    return mBind((((opt.argv.length === 0) && ((Object.keys(opt.options)).length === 0)) ?
        ...
        return mBind(((true === opt.options['version']) ?
            ...
            ((true === opt.options['browser-bundle']) ?
                (function() {
                    var bundle = require.resolve("elfenben/lib/browser-bundle.js");
                ...
                ((true === opt.options['run']) ?
                    (function() {
                        var infile = (opt.argv[0] ?
                            (((opt.argv[0].indexOf('.elf') === -1) && (opt.argv[0].indexOf('.js') === -1)) ?
                                error(new Error("Error: Input file must have extension '.elf' or '.js'")) :
                                opt.argv[0]) :
                    ...
                    ((true === opt.options['watch']) ?
                        (function() {
                            var cwd = process.cwd();
                            console.log('Watching',cwd,'for .elf file changes...');
                            watch.watchTree(cwd,{
                                filter: function(f,stat) {
                                    return (stat.isDirectory() || (f.indexOf('.elf') !== -1));
                                },
                                ignoreDotFiles: true,
                                ignoreDirectoryPattern: /node_modules/
                            },function(f,curr,prev) {
                                return ((curr && (curr.nlink !== 0)) ?
                                    (require("child_process")).spawn("elfenben",[f.substring(cwd.length+1)],{stdio: "inherit"}) :
                                    (((Object.prototype.toString.call(f) === "[object Object]") && (prev === null) && (curr === null)) ?
                                        (function(o,f,s) {
                                            ...
                                            return ((!(initialf === cwd)) ?
                                                (function() {
                                                    return (function() {
                                                        return (require("child_process")).spawn("elfenben",[initialf.substring(cwd.length+1)],{stdio: "inherit"});
                                                    })();
                                                })() :
                                                undefined);
                                        }) :
            ...
            return mBind((opt.argv[0] ?
                ...
                return mBind((function() {
                    ...
                    ((!outfile) ?
                        (function() {
                            return (function() {
                                outfile = infile.replace(/\.elf$/,".js");
                                return ((outfile === infile) ?
                                    error(new Error("Error: Input file must have extension '.elf'")) :
                                    undefined);
                            })();
                        })() :
                        undefined);
                    ...
                    return (function() {
                        ...
                        try {
                            console.log('Elfenben v1.0.1:  compiling',infile,'to',outfile);
